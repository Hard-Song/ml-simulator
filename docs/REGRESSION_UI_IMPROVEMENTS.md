...
为分类任务和回归任务的数据集特性和模型能力配置添加详细的参数说明和调参建议，帮助用户更好地理解每个参数的含义和合理取值范围。
...
##### 1.1 类间可分性 (Separability)
- **参数含义**: 不同类别特征之间的可区分程度。越高表示类别越容易分开
- **取值范围**: 0 - 1
- **调参建议**:
  - 0.6 - 0.8: 为正常难度
  - > 0.9: 适合测试弱模型
...
##### 1.2 标签噪声 (Label Noise)
- **参数含义**: 标签被错误标记的比例。一部分样本的标签会被随机翻转
- **取值范围**: 0 - 0.5
- **调参建议**:
  - 0.02 - 0.1: 为正常范围
  - 0.15 - 0.3: 模拟标注质量差
...
##### 1.3 特征噪声 (Feature Noise)
- **参数含义**: 特征向量X中添加的噪声强度。降低特征的信息量
- **取值范围**: 0 - 0.5
- **调参建议**:
  - 0.05 - 0.2: 为正常范围
  - > 0.3: 需要强特征工程
...
##### 1.4 非线性强度 (Nonlinearity)
- **参数含义**: 决策边界的非线性程度。越高需要模型有更强的非线性表达能力
- **取值范围**: 0 - 1
- **调参建议**:
  - 0 - 0.3: 适合线性模型
  - 0.3 - 0.6: 需要树模型
  - > 0.6: 需要DNN/Transformer
...
- **低可分性** → 提高 `capacity`
- **高标签噪声** → 提高 `noise_tolerance`
- **高非线性** → 提高 `capacity`
- **欠拟合 (低Acc)** → 降低 `bias`，提高 `capacity`
- **过拟合 (训练Acc高但测试Acc低)** → 降低 `variance`，提高 `noise_tolerance`
...
### 场景1: 测试线性分类模型 (Logistic Regression)
...
**预期结果**: 高noise_tolerance的模型(如RF)表现更好
